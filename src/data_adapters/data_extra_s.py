...
_dir_raw = "D:\Projekti\DataSets\Datasets_Activity\ExtraSensory_Dataset\\"
_dir_processed = "D:\Projekti\DataSets\Datasets_Activity\ExtraSensory_Dataset\\Processed\\"
...
_labels_set3 = ["LYING_DOWN", "SITTING", "STANDING_IN_PLACE", "WALKING", "STAIRS_-_GOING_UP", "STAIRS_-_GOING_DOWN",
                "RUNNING", "BICYCLING", "ELEVATOR", "DRIVE_-_I_M_THE_DRIVER", "LIFTING_WEIGHTS", "COOKING", "CLEANING",
                "VACUUMING", "WASHING_DISHES", "SMOKING", "DANCING", "JUMPING", "EATING", "SLEEPING"]
_label_names = ['LYING_DOWN', 'SITTING', 'STANDING_IN_PLACE', 'STANDING_AND_MOVING', 'WALKING', 'RUNNING', 'BICYCLING',
...
def process_directory(src_dir, dst_dir, list_labels_of_interest):
    ...
    :param dst_dir: destination path for test and train files
    :param src_dir: source path with raw data
    :return: features, labels
    """
    count = 0
    features = []
    acc_dir = src_dir + "Raw\\raw_acc\\"
    gyro_dir = src_dir + "Raw\\proc_gyro\\"
    ...
    x = np.empty((1, _feature_number))
    y = np.empty(1)
    sp.set_global_vars(_freq, _window, _overlap, _tr_test_split_coef)

    # load labels
    label_timestamp = read_label_timestamp_list(dst_dir + "..\\timestamps_labels.txt")
    ...
    for lab in list_index:
        ...
        for (uuid, t) in tuples:
            ...
            if os.path.isfile(gyro_file):
                ...
                print("SKIPPED: Gyro_mobile File %d for timestamp %d and uuid %s does not exist!" % (count, t, uuid))
                continue
            ...
            file_features = sp.process_raw_data(acc, proc_gyro)
            if file_features.shape[0] != _feature_number:
    ...
    print("Shape of labels array after: " + str(y.shape))
    print("User uuid: %s has labels %s" % (uuid, str(util.count_occurance(y))))

    y = util.convert_labels_to_norm(y, list_index)
    if not os.path.isdir(dst_dir):
        os.mkdir(dst_dir)

    np.savetxt(dst_dir + "Y_ALL.txt", y, "%d")
    np.savetxt(dst_dir + "X_ALL.txt", x, "%.9f")
    return x, y
